...
- `invoiceAI.js` — `POST /api/fatura-analiz`: multer **memoryStorage** (file never touches disk), 15MB / pdf+jpg+png+webp whitelist, Anthropic `claude-opus-4-8` with `output_config.format` json_schema (no temperature — 400s on Opus 4.8). Returns parsed JSON only; saving always goes through user-approved `POST /api/expenses`. Requires `ANTHROPIC_API_KEY`. Şema tevkifat alanlarını da çıkarır (tevkifat_var/orani/tutari/sebebi/odenecek_tutar).
- **KDV Tevkifatı:** `expenses.tevkifat_orani` ("2/10" gibi), `tevkifat_tutari` (devlete sorumlu sıfatıyla ödenen = hesaplanan KDV × oran), `odenecek_tutar` (satıcıya fiilen ödenen = brüt − tevkifat). `hesaplaTevkifat()` oranı "pay/payda" olarak parse eder. **Cari borç = `SUM(COALESCE(odenecek_tutar, brut_tutar))`** — tevkifatlı faturada satıcıya brüt değil ödenecek tutar borçludur, banka ödemesi böylece mutabık olur. P&L'de gider hâlâ net üzerinden (tevkifat P&L'i etkilemez; sadece KDV'nin kime ödendiğini değiştirir). Tevkifat tüm kalemlere aynı oranda uygulanır (KDV'yle orantılı olduğu için satır toplamları doğru toplanır).
